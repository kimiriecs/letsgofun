      ...
      <div class="form-wrapper col-10 col-sm-6 d-flex flex-column">
        <form class="form p-3 flex-fill">
          ...
          <div class="d-grid mx-auto">
            <button type="submit" @click.prevent="createOrder(order)" class="btn btn-outline-success w-100">
              Create
            </button>
          </div>
        </form>
...
export default {
  ...
  methods: {
    ...
    closeOrder(event) {
      this.$modal.hide("dashOrderCreate");
    },
    beforeClose(event) {
      // if (this.validationErrors) {
      //   this.clearValidationErrors();
      // }
      this.$router.push({ name: "dashOrders" });
    }
  },
  mounted() {
    this.$modal.show("dashOrderCreate");
  }
};
</script>
...
.vm--overlay[data-modal="dashOrderCreate"] {
